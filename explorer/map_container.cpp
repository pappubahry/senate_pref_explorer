...
Map_container::Map_container(QWidget* parent)
  : QQuickWidget(parent)
{
}
...
void Map_container::mouseMoveEvent(QMouseEvent* event)
{
  ...
  const QSize map_size = size();
  const int size_x     = map_size.width();
  const int size_y     = map_size.height();

  const int x               = event->pos().x();
  const int y               = event->pos().y();
  const double mouse_tile_x = _tile_x + (x - size_x / 2) / 256.;
  const double mouse_tile_y = _tile_y + (y - size_y / 2) / 256.;

  const double lon = 360. * mouse_tile_x / zoom_factor - 180.;
  const double n   = M_PI - 2. * M_PI * mouse_tile_y / zoom_factor;
  const double lat = _rad2deg * atan(sinh(n));

  const double d_lon = 360. / (256. * zoom_factor);
  const double d_lat = 2. * M_PI * cosh(n) / ((1 + sinh(n) * sinh(n)) * zoom_factor);

  emit mouse_moved_2arg(lon, lat);
  emit mouse_moved_4arg(lon, lat, d_lon, d_lat);
}
...
void Map_container::update_coords()
{
  ...
  _tile_y    = pow(2., _zoom - 1.) * (1. - log(tan(_latitude / _rad2deg) + 1. / cos(_latitude / _rad2deg)) / M_PI);
}
